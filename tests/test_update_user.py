...
class TestUpdateUser:
    ...
    def test_update_authorized_user(self, generate_user_data):
        UserMethods.create_user(generate_user_data[0])
        UserMethods.log_user(generate_user_data[1], generate_user_data[2], generate_user_data[3])
        ...
        update_user = UserMethods.update_data_user(user_token, body)
    ...
    def test_update_already_existed_email(self, generate_user_data):
        UserMethods.create_user(generate_user_data[0])
        UserMethods.log_user(generate_user_data[1], generate_user_data[2], generate_user_data[3])
        user_token = UserMethods.user_token_by_user_data(generate_user_data[1], generate_user_data[2],
                                                         generate_user_data[3])
        with allure.step('В переменную с новыми данными передать email, который уже используется'):
            ...
            update_user = UserMethods.update_data_user(user_token, update_body)
    ...
    def test_update_unauthorized_user(self, generate_user_data):
        UserMethods.create_user(generate_user_data[0])
        body = generate_update_user_body()
        update_user = UserMethods.update_data_unauthorized_user(body)
